...
if __name__ == '__main__':
    ...
    result = connection.execute("""
        ...
        """).fetchall()
    print('1) Названия сборников, в которых присутствует 2 PAC: ', result)

    result = connection.execute("""
            SELECT col_title FROM collection
            JOIN trackscollection tc ON tc.col_id = collection.id
            JOIN tracks t ON t.id = tc.track_id
            JOIN albums a ON a.id = t.album_id
            JOIN singersalbums sa ON sa.album_id = a.id
            JOIN singers ON singers.id = sa.singer_id
            WHERE singers.singer_name = '2 PAC';
            """).fetchall()
    print('2) Названия сборников, в которых присутствует 2 PAC: ', result)
    ...
    result = connection.execute("""
        ...
        """).fetchall()
    print('1) Исполнитель(ли), написавший(ие) самый короткий по продолжительности трек: ', result)

    result = connection.execute("""
                SELECT singer_name FROM singers
                JOIN singersalbums sa ON sa.singer_id = singers.id
    ...
    result = connection.execute("""
            ...
            WHERE id IN (SELECT album_id FROM tracks
                        ...
                        LIMIT 1);
        """).fetchall()
    print('1) Названия альбомов, содержащих наименьшее количество треков: ', result)
    ...
    result = connection.execute("""
            ...
            LIMIT 1;
        """).fetchall()
    print('2) Названия альбомов, содержащих наименьшее количество треков: ', result)